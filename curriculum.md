...
## THREAD
baton: 097 opened on 095 and 096 as two failures that never threw anything you could see, and asked what you would have had to write down to catch either. the establishing move is a log line for one model call with the http fields on top and the model-specific ones under a gap, each carrying the question it answers: model id, prompt version, tokens in and out, cached tokens, finish reason, attempt number, request id. the click is that model looks like a constant sitting in your config but an alias moves under you (095) and a fallback answers from somewhere else (093), so the id that replied isnt always the one you asked for. tokens_in with cached_in is what tells a spend spike caused by more calls apart from one caused by fatter prompts, and attempt is what shows retries eating the latency budget (087) before a user does. the caveat is that logging the prompt and answer text is where user data ends up, so its a decision and not a default, flagged forward to arc 9 without building it. the note ended pointing forward.

next is the arc 8 CAPSTONE, "what happens after deploy, keeping an llm feature alive". it assembles, it does not teach: every brick from 082 to 097 has to appear as a labelled part of one picture, carrying its note number, and nothing new gets built. the natural shape is the life of one deployed feature, the request path (084, 085, 086), the failure path (088 to 094), the change path (095, 096) and the seeing-it path (097), with the eval harness from 081 hanging off the change path. run the capstone gap check first: if the assembly needs a brick that was never laid, add one checkbox before the capstone and write that instead. capstone budget is up to 350 prose words. last two visuals were an annotated json log line (097) and a mermaid flowchart (096), and a capstone visual is an assembly diagram, so mermaid is correct here despite 096.

caches stay laid and closed. 085 owns exact prefix caching, 086 owns fuzzy question matching, 087 owns the two budget numbers, 088 owns the meters, 089 owns backoff, 090 owns jitter, 091 owns retryable vs never, 092 owns the timeout number, 093 owns the second provider, 094 owns what the user gets when it is all gone, 095 owns alias vs dated snapshot, 096 owns the retirement date and the migration window, 097 owns the log line fields for a model call. bm25 scoring is still an unlaid brick on purpose, dont let arc 8 or 9 drag it in.

process note: 085 at 179, 086 at 234, 087 at 207, 088 at 223, 089 at 212, 090 at 202, 091 at 196, 092 at 172, 093 at 192, 094 at 201, 095 at 229, 096 at 206, 097 at 216. the 176 to 215 band is where this arc reads best.

last visuals: annotated artifact, one json log line for a model call with the http fields on top and the model-specific fields under a gap, each commented with the question it answers (097), mermaid flowchart, one deprecation window from the pin to the retirement day, branching into the migration loop and into the do nothing path (096), annotated artifact, one model field written both ways and then resolved on two dates, with the alias moving and the snapshot not (095).
last exits: forward (097), stops (096), forward (095). the capstone must stop, not point forward.
