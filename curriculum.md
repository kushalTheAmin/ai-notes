...
baton: 129 put a second model in front of a saved run and had it grade the path instead of the ending. its worked example is a grade prompt with five numbered steps pasted into it, two of them the identical search, and a fifth step where the agent writes a reason, damaged item, that appears in nothing it read. three rubric lines come back, finished pass, no repeat fail, backed fail. it makes three moves. first, it names the hole 127 and 128 leave, both of them need a run that ends in a row you can look up, and this run ends in a paragraph. second, it says whats actually new next to 074, not the call and not the json, the input, an answer becomes a whole path. third, it carries 075s tilt over flat, a long trace reads as careful the same way a long answer did, so the token column from 128 has to sit next to the verdict. its caveat is that the judge only knows what you paste in, and its the tool results that make the backed question answerable, which is 116 territory, the prompt gets fat fast. the next checkbox is the judge moves in front of the gate, a filter before a person sees it. it should pick up that 129s call is still a measurement, run after the fact on a saved run, and that moving the same call to before the action turns it into a control that costs latency on every risky call. it sits in front of 125s pause, so its job is shrinking the pile that reaches a person, never replacing them, and it has to fail closed, and it reads the same context 123 poisoned.
...
last visuals: 15 line pseudocode grade prompt with a five step trace pasted into it, plus a 3 line json verdict block (129), flat plain text worked example, five run lines of rounds and tokens plus three summary lines (128), three column comparison table, four rows, the runs last message against the refunds table after it against a score (127). still no diagram since 125s flowchart, and the gate note is about where a call sits in a sequence, so it is the natural place for mermaid.
last exits: forward (129), stops (128), stops (127). one of the last three points forward, so the next note is free either way.
