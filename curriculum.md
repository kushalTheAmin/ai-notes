...
baton: 121 closed arc 10 by assembling 109 through 120 into one loop picture, the toolbox arriving as text, the run that can be an api or a screen, the append that takes errors too, the five notes that keep the array from growing forever, and an exit gated by a check my own code answers. it taught nothing new, it linked, and it deliberately carried no worked numbers at all since every number in this arc belongs to a brick. it landed on the gap the arc left open, every check in that picture asks did it work and not one of them asks whether the call should have run. the next checkbox is arc 11s first, read vs write, blast radius decides which calls need a gate. that is an arc opener so it builds on 121 and it should pick up that exact gap sentence rather than re-establishing what an agent is. blast radius is not a built term yet, arc 11s checkbox uses it but no note has, so the note that first says it has to say what it means in the same sentence.

for the record, what 121 established: a mermaid flowchart assembly diagram, nine nodes, the toolbox feeding into the model, a tool call fork running to run it then append the result then keeping the array small and back to the model, and the no branch running into the did it actually happen check with two exits, return the answer and return failed. every brick named by its number, with 115 through 119 collapsed into one node the way the overlap note asked. no worked numbers anywhere in it. its real world detail is the closing line about issue_refund going through because my code was already holding the wire. it stops, it states the gap as a fact rather than previewing arc 11.
...
process note: 108 at 314 under the 350 capstone ceiling, 109 at 214, 110 at 232, 111 at 193, 112 at 222, 113 at 189, 114 at 198, 115 at 190, 116 at 187, 117 at 197, 118 at 207, 119 at 203, 120 at 205, 121 at 326. 121 drafted at 339 and came down by trimming five phrases, no brick lost its mention. both capstones landed in the 310 to 330 band and read fine, aim there rather than at the ceiling.

last visuals: mermaid flowchart assembly diagram, the toolbox and the loop and the two exits, nine nodes (121), mermaid sequence diagram, model and my code and the screen, a screenshot in and a coordinate out over six messages (120), plain text worked example, two labelled scenarios, three calls totting up to 10,200 tokens in the parents window against the same job returning one sentence from a sub-agent (119). the last two are both mermaid, so the next note must not be mermaid. a comparison table or a worked example, and read vs write is a two column comparison by its nature.
last exits: stops (121), forward (120), stops (119). only one of the last three points forward, so the next note is free either way.
