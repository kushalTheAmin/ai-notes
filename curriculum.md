...
baton: 123 took the ranking 122 built and showed what aims at it. the attack is 052s exactly, text my code fetched and pasted in as context, and the only thing that changed is the consequence, the model now writes a tool call instead of a sentence and 113s append path means my code has already run it. the payoff is that the blast radius of an injected line isnt bounded by the tool that fetched it, every tool in the box is in range, so an attacker aims at the rows 122 said cant be taken back. it closed by saying which tools can run on their own and which need a person is the whole game now, and that neither of the next two notes is about detecting the injection. the next checkbox is cap the tool, not the model, least privilege for agents. it should pick up that closing line from the scoping side, the tool is handed less power to begin with, and leave the human in the path to checkbox 4. careful, 123 already used "out of reach of a followed instruction" as the framing, so the scoping note builds on it rather than announcing it as new.

for the record, what 123 established: a plain text annotated artifact in two halves, the tool result my code appended after a search_docs call with a poisoned html comment inside a wiki page, and the tool call the model writes on the same round, each with a caret annotation under it. the injected line aims at email_customer with a link, deliberately, because 122 put email_customer in the cant take it back column while issue_refund is the reversible one. it reuses order 88120 from 052 and the tool names search_docs, list_recent_orders and email_customer. no arithmetic in it. its honest reaction is realising 052 already carried the fix and i had filed it as chat app advice. it points forward, it names the next two notes without previewing their content.

previously, what 122 established: it opened arc 11 by picking up 121s closing line, go find the line in your agent that says no, and answering the smaller question underneath it, which calls even need one. it laid blast radius as a term, defined in the same sentence it appears, how far a wrong call reaches past my own code and how much of it sticks. the payoff is that read vs write is the wrong axis, a wrong get_order_by_id is a read that puts one persons order in front of another and cant be taken back, while issue_refund is the scariest name on the list and is the recoverable one. it deliberately did not design the gate, checkbox 4 owns that, and it did not touch permissions, checkbox 3 owns that. the visual was a four row three column comparison table, the call, what a wrong one leaves behind, can i take it back. rows are search_docs leaving nothing but spent tokens, get_order_by_id with a wrong id leaving one customers order on another ones screen with no, issue_refund leaving money out of the account with yes a second call reverses it, and email_customer leaving a mail somebody already read with no. no arithmetic in it at all, the numbers in this arc are going to be step counts and budgets later. it reuses the made-up tool names the repo already has and adds one, email_customer. its real world detail is a GET handing back the wrong users record still being an incident in my .NET work. it stops, it lands on the sort rule and bookends its own opening line rather than previewing the gate.

previously, what 121 established: a mermaid flowchart assembly diagram, nine nodes, the toolbox feeding into the model, a tool call fork running to run it then append the result then keeping the array small and back to the model, and the no branch running into the did it actually happen check with two exits, return the answer and return failed. every brick named by its number, with 115 through 119 collapsed into one node the way the overlap note asked. no worked numbers anywhere in it. its real world detail is the closing line about issue_refund going through because my code was already holding the wire. it stops, it states the gap as a fact rather than previewing arc 11.

so arc 10 is closed and arc 11 is two bricks in. the repo now has an agent that can be described to, called, recovered when a call fails, stopped when it thinks its finished and isnt, and kept inside its window by four different moves plus delegation, and as of 122 it has a way to rank its own tools by what a wrong call costs, and as of 123 a reason that ranking is urgent rather than tidy. what it still has nowhere is anything that acts on it. no scoping, no gate, no trace, no budget. those are the next four checkboxes and they should be written as if the table in 122 and the poisoned wiki page in 123 are both already on the readers desk.

overlap to keep an eye on for arc 11: 050 already appends a broken reply plus an error string and retries, for the models own malformed json, and 113 is the same move aimed at a tool that failed. if a note goes near retries again, name the difference explicitly. second one, arc 11 has "grade the end state, did the thing actually happen", and 114 deliberately stayed on the runtime return path, the check my code runs before the loop hands anything back. arc 11s note is measurement after the fact, scoring a finished run the way arc 7 scores an answer, so open it on that difference. third one, 119 said the sub-agent comes back with a sentence you cant audit, and arc 11s "a trace, not a log line" owns the fix, 119 only named the problem. fourth one, 097 already itemized what a log line for one model call needs, so the trace note is about a run of twenty calls rather than about logging fields. fifth one, that pairing is spent, 123 wrote it. 052 owns injection where the damage is what the model says and 123 owns it where the damage is what my code does, so no later note in this arc re-explains the attack, they use it flat. 123 also already said the fix is keeping the dangerous thing out of reach of a followed instruction, so checkboxes 3 and 4 supply the two halves of that rather than announcing the idea. sixth one, 122 owns blast radius and owns the read vs write correction, so no later note in this arc redefines either, they use the term flat. 122 also deliberately left the gate itself alone, so "the gate that doesnt drown you" still has all of its own material, and it left scoping alone so "cap the tool, not the model" has its.
...
process note: 108 at 314 under the 350 capstone ceiling, 109 at 214, 110 at 232, 111 at 193, 112 at 222, 113 at 189, 114 at 198, 115 at 190, 116 at 187, 117 at 197, 118 at 207, 119 at 203, 120 at 205, 121 at 326. 121 drafted at 339 and came down by trimming five phrases, no brick lost its mention. both capstones landed in the 310 to 330 band and read fine, aim there rather than at the ceiling. 122 at 219, 123 at 227.

last visuals: plain text annotated artifact, a poisoned tool result and the tool call it produced, two blocks with a caret under each (123), comparison table, four calls by three columns, what a wrong one leaves behind and whether it can be taken back (122), mermaid flowchart assembly diagram, the toolbox and the loop and the two exits, nine nodes (121). an artifact, a table and a mermaid in the last three, so nothing is blocked. cap the tool, not the model is two versions of the same api key or the same tool definition side by side, so a tiny comparison table or a short pseudocode block both fit, and a second annotated artifact in a row would be the dull pick.
last exits: forward (123), stops (122), stops (121). only one of the last three points forward, so the next note is free either way.
