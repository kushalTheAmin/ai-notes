...
baton: 115 opened one round of the doc-QA loop and itemized what gets posted. round 3 sends 2,974 tokens, of which the question is 14, the models own two tool-call replies are 60, and 2,600 is text my tools handed back. the two agent-only lines are the schema block riding along every round whether or not anything gets called, and every tool result staying in the array in full. 116 picks up exactly there: the results dominating the payload, and what you do about it, truncate or paginate. 116 is the window problem (006), not the bill, so open it on the ceiling rather than re-running 115s money angle. still no gating, no permissions, arc 11 owns those.

for the record, what 115 established: a markdown table, three columns, part of round 3s array against tokens against who wrote it. numbers are 120 system, 180 schema, 14 question, 30 and 30 for the two model replies, 1,300 and 1,300 for two four-chunk results, total 2,974. it named the difference from 085 out loud, 085s single call had most of its bill in a cacheable stable prefix and here the stable part is only 314 tokens, under 085s own roughly-a-thousand cache minimum, while the two big lines are new every round. it pointed forward at 116 by name.

previously, what 114 established: a 14 line pseudocode block, 082s loop with its exit line opened up, a comment showing the text it can return with issue_refund nowhere in the messages array, and a refund_exists lookup under it. order 44718, the real id 113 ended on, not 4471. it ended on the honest limit, no forward pointer.

so as of 115 the arc has a described toolbox, a place to fetch it from, a loop that recovers from a failed call, a done check that does not ask the model, and an itemized bill for one round. still no check anywhere on whether a call should happen.

overlap to keep an eye on for 116: 115 has now spent the tool-result-size observation once, 2,600 of 2,974, so 116 cannot arrive at that same reveal again. 116 starts from it as a given and spends its words on the fix, cutting a result down or handing it back a page at a time, and on what breaks when the model only sees part of what the tool found. 058 owns chunk size for retrieval, thats a different cut and worth naming if the note goes near it.

overlap to keep an eye on: 050 already appends a broken reply plus an error string and retries, for the models own malformed json. 113 is the same move aimed at a tool that failed, inside the agent loop, and it stayed short rather than spend words drawing that line. if a later note in this arc goes near retries again, name the difference explicitly instead of letting the two blur. second one, arc 11 has "grade the end state, did the thing actually happen". 114 deliberately stayed on the runtime return path, the check my code runs before the loop hands anything back. arc 11s note is measurement after the fact, scoring a finished run the way arc 7 scores an answer. when that note gets written, open it on that difference.
...
process note: 108 at 314 under the 350 capstone ceiling, 109 at 214, 110 at 232, 111 at 193, 112 at 222, 113 at 189, 114 at 198, 115 at 190. three short ones in a row now, all landed, all by picking a concept with one moving part rather than by trimming. keep 116 under 210 and treat the 220s as a signal the checkbox wanted splitting.

last visuals: markdown table, three columns, one rounds payload itemized by line and by who wrote it (115), 14 line pseudocode block, 082s loop with the exit line opened up and a database check written under it (114), plain text annotated artifact, an eight line messages array with two arrows on it (113). nothing is running three deep. 116 should not be a table, thats two running and the shape is spent on this exact payload.
last exits: forward (115), stops (114), forward (113). two of the last three point forward, so 116 has to just stop.
