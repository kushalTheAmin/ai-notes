...
## THREAD
baton: 077 put the row-level diff next to the headline. same golden file from 072, prompt A 41/50 to prompt B 44/50, and that +3 is 5 rows fixed and 2 broken. the point it established: a score is a sum with no memory of which rows built it, so +3 and "5 fixed, 2 broke" are the same number and only the diff separates them. it named #22 as the real regression, a row that passed on the old prompt and broke on the new one with nothing throwing, and it reused #7, the row that self-flipped in 076, to land the one procedural line it allowed itself, re-run a flipped row on both prompts before calling it a regression. it deliberately built no further procedure than that, and it framed the habit as reading eval runs like a pull request diff.

the next checkbox is "evals in ci: treating prompts like code". 077 already handed it the diff habit and the pull-request framing, so the run belonging to the commit is a short hop and does not need re-arguing. the shape to aim for: the golden run wired to the change that touches a prompt, and the part thats genuinely awkward, a unit test returns a boolean and this returns a number, so what exactly fails the build. 076 and 077 have both already been spent on why that threshold is hard, so this note assumes noise and assumes per-row regressions, it does not re-derive either. the honest tension to sit in is that a strict "score must not drop" gate trips on the 2 point wobble 076 measured, and a loose one lets #22 through. if the draft starts building a whole threshold policy, plus caching, plus cost per run, thats the split rule firing and it becomes two or three checkboxes.
...
process note: 070 at 324 on the capstone ceiling, 071 at 190, 072 at 221, 073 at 191, 074 at 202, 075 at 218, 076 at 200, 077 at 208. the band called for 077 was 160 to 210 and it landed at 208, with the diff block carrying the counts and the prose only naming two rows. 078 is a ci note, so the artifact will likely be a config or a pipeline and the prose has to stay off it. 160 to 210 again.

last visuals: annotated artifact, an eval diff between two runs with the fixed list, the broke list and the net printed (077), worked example, two runs of one prompt over the same 50 rows with the four flipped row ids listed (076), comparison table, one answer four ways against a soft rubric (075). nothing is at three in a row, but 077 was already an artifact-shaped block, so 078 should lean mermaid or pseudocode rather than dissecting another file.
last exits: stops (077), forward (076), stops (075). only one of the last three points forward, so 078 may go either way.
