...
baton: 104 laid the accept-the-error-rate brick. the visual was a four row table, one column naming where the answer lands, one naming who catches the wrong one, one naming what a single wrong answer costs, all four rows the same model at the same 95%. the walk was 200 requests a day at 95% right is 10 wrong answers a day forever, and the knob that moves isnt accuracy, its where the answer lands and whether anyone with a way to check sees it before it takes effect. the click is that the fix for a bottom-half feature is usually moving it up a row, a confirm step or shown sources, not a better prompt. the caveat is that 5% isnt assumable, you measure your own (072) and it wont spread evenly. it opened by separating itself from 094, the call dying versus the call working and the answer being wrong. exit pointed forward at rent versus own.

next is "api models vs open weights: renting a model vs owning one". 104 ended on the first question in this arc that isnt about the request at all, so 105 picks up that widening and asks who runs the weights. 025 already established that an open model is a folder with one huge file of numbers in it and 026 established that every one of those numbers gets read per token, so the cost and hardware side has real bricks under it already. keep it to the decision, what you get and what you take on, and do not name a specific model or vendor, model selection later in this arc owns comparisons. 095 and 096 own the pinning and deprecation angle, so touch that only as one consequence of renting, dont re-teach it.

caches stay laid and closed, arc 8 owns all of them and 098 owns the assembly, dont let arc 9 reopen a mechanism. bm25 scoring is still an unlaid brick on purpose. 099 through 104 have all held the no-named-vendor line, 105 is the one where that gets hard and it still holds. arc 7 owns measurement, 104 pointed at it in one clause and went no further, keep it that way.

process note: 097 at 216, 098 at 324 as a capstone, 099 at 217, 100 at 216, 101 at 182, 102 at 206, 103 at 193, 104 at 233. nothing has touched 250 in a while, good. 105 should come in under 220.

last visuals: table, four rows and three columns, same model down every row with the landing place changing (104), mermaid flowchart, eight nodes, one fork into two branches with two diamonds on one side (103), worked example, one sentence traced through five labeled steps with a two-row map (102). 105 must not be a table or mermaid, an annotated artifact or a comparison traced with real numbers would both be fresh.
last exits: forward (104), stops (103), stops (102). 104 broke the run of stops, so 105 may go either way.
