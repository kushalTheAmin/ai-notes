...
## THREAD
baton: 078 took 077s diff habit and wired it to the commit. the golden run (072) fires on any change touching the prompt file, and the thing it settled is what turns the build red: not the score, the rows. any id that passed on the last green run on main and fails now is red, and each one has to prove it first, re-run on both prompts a few times so a #7 style self-flipper doesnt paint the build red on its own. the line it landed on is that a score that drops is already a row that regressed, theres no other way for a total to fall, so the row gate catches everything a score gate would plus the breaks hiding inside a net win. it printed the score in the log and gated nothing on it. it deliberately built no threshold policy, no caching, no cost per run, those stay unlaid.

the next checkbox is "error analysis: stop guessing, read the failures and bucket them". 078 hands it a red build carrying a list of ids and nothing else, so the hop is what you actually do when you open that list, and the opener should pick up exactly that. the shape to aim for: the failing rows read one at a time and dropped into three or four named piles, the retrieval never returned the chunk vs the chunk was right there and the answer ignored it vs the golden row itself was wrong, and the point that the pile counts tell you which lever to pull while the score tells you nothing about where to work. arc 6 already laid the retrieval failure modes, 062 for the vocabulary gap and 067 for recall, so a pile can name one without re-deriving it. if the draft starts growing a taxonomy of eight or ten categories, thats the split rule firing. three piles with real row ids sitting in them is the note.
...
process note: 070 at 324 on the capstone ceiling, 071 at 190, 072 at 221, 073 at 191, 074 at 202, 075 at 218, 076 at 200, 077 at 208, 078 at 215. 078 came in at 227 on the first draft and the cuts that got it to 215 were all repetition between the prose and the code comments, which is the usual overspend on a pseudocode note. 079 is a sorting note and the piles want to live in the visual, so 160 to 210 again.

last visuals: pseudocode, a ci job that computes the regressed rows and exits on them (078), annotated artifact, an eval diff between two runs (077), worked example, one prompt run twice over the same 50 rows (076). nothing is at three in a row. 079 wants its piles in the visual, so a small table or a worked list of real failing rows fits better than another code block.
last exits: stops (078), stops (077), forward (076). two stops running, so 079 pointing forward is welcome and allowed.
