...
baton: 126 answered the question 125 left open, which of those decisions actually got written down anywhere. it made the run the unit of observation instead of the call, one id created at the start of the run and threaded into every call it makes, each step recording which step spawned it, so a sub-agents rounds hang under the parent step that launched them. the point it lands is that the parent of that run never sees an input over 1,850 tokens while the biggest call in it is 5,800 one level down, so a flat log of individually correct lines cannot show you where a run went wrong. the next checkbox is grade the end state, did the thing actually happen. it should pick up that 126 gave the run a readable shape but every judgement in it is still mine, reading. per the overlap list below it must open on the difference from 114, which stayed on the runtime return path my code checks before the loop hands anything back, where the arc 11 note is measurement after the fact, scoring a finished run the way arc 7 scores an answer.
...
process note: 108 at 314 under the 350 capstone ceiling, 109 at 214, 110 at 232, 111 at 193, 112 at 222, 113 at 189, 114 at 198, 115 at 190, 116 at 187, 117 at 197, 118 at 207, 119 at 203, 120 at 205, 121 at 326. 121 drafted at 339 and came down by trimming five phrases, no brick lost its mention. both capstones landed in the 310 to 330 band and read fine, aim there rather than at the ceiling. 122 at 219, 123 at 227, 124 at 244, 125 at 249. 124 drafted at 228, then went up when the 111 cross reference went in, it was worth the words. 125 drafted at 254, over the ceiling, and came back under by tightening two sentences rather than dropping the caveat about a tool that fires constantly. 126 at 222, drafted at 230 and trimmed four phrases, not because it was over but because 122 through 125 all landed in the 219 to 249 band and sitting there every run is its own tell. the next few notes should aim lower, a 160 to 190 word note is overdue.

last visuals: plain text annotated artifact, an indented 12 line trace of one run with tokens and timings and three caret annotations (126), mermaid flowchart, eight nodes, two filters and a pause branch (125), pseudocode block, two versions of email_customer, wide and capped, 13 lines (124). artifact, flowchart and pseudocode in the last three, so nothing is blocked, but 123 and 126 are both annotated artifacts and thats the type this arc keeps reaching for. grade the end state is a comparison, what the run said happened against what the database says happened, so a small table or a two block before and after fits it better than another indented listing.
last exits: forward (126), stops (125), stops (124). one of the last three points forward, so the next note is free either way.
