...
baton: 135 took the first half of the budgets and kill switches split, the automatic cap. its visual is a flat plain text worked example, one run against two caps, 8 rounds and 20,000 tokens, six round lines with a per-round cost and a running total, climbing to 22,400 at round 6 where the crossing is marked, and a closing line saying the run stopped with nothing written. it makes one move and adds a price. the move, the counter is a variable my own code adds to at the top of every round and the stop is an if that breaks the loop, no model anywhere in it, and 114 already put a check outside the model so this one sits in the same place asking a different question, have you spent enough rather than are you finished. it also spends the per run not per request line from the checkbox in one clause, one call is cheap and predictable, fifteen with a growing history is not. the price, the cap has no idea where in the run it lands, so the clean stop in the example is luck and the next one falls between a write and its follow-up, which uses 122 flat rather than re-explaining it. the next checkbox is the kill switch, stopping the runs already going. that is the manual half, a flag living outside every run that a person flips and every loop reads at the top of a round, and its own subject is the runs already in flight, the ones a deploy or a config change does not reach. 135 has spent the counter, the trip, whichever-line-first and the mid-run stop being survivable, so the kill switch note cannot re-run any of those, it is about a stop that comes from outside a run and hits all of them at once. after that comes when not an agent, and 130 owns putting a check before an action and 132 and 134 own memory going stale and memory outliving a delete, so none of that is available to it.
