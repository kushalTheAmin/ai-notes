...
baton: 134 took the second half of that split, how long a row stays readable. its visual is a mermaid flowchart, five nodes, a delete request for customer 4471 fanning out to three stores i wrote it against, orders, transcripts and my own request logs from 103, with the memory row sitting on no arrow from that delete at all and a dotted one going into round 1 of every later run. it makes one move and adds a price. the move, the row is a second copy that no cleanup path knows about, written in a different corner of the code at the end of a run by a model, so no delete reaches it and nothing dates it, and the thing that made it land is that it isnt a cache, a cache has a real version behind it and this one has none, once the transcripts go the row is the only copy left. the price, an expiry means picking a window and accepting the agent forgets things worth remembering. it inherits 133s key and 132s wrong row flat and re-argues neither, and it names 103 to stay off provider retention. the next checkbox is budgets and kill switches, caps per run not per request, which is 128s counting already done and 125s auto versus ask already spent, so it is only about a counter crossing a line mid run, inside one run, with no model in it and nothing carried between runs. after that comes when not an agent, and 130 owns putting a check before an action and 132 owns memory going stale, so neither is available to it.
