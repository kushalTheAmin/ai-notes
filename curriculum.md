...
baton: 122 opened arc 11 by picking up 121s closing line, go find the line in your agent that says no, and answering the smaller question underneath it, which calls even need one. it laid blast radius as a term, defined in the same sentence it appears, how far a wrong call reaches past my own code and how much of it sticks. the payoff is that read vs write is the wrong axis, a wrong get_order_by_id is a read that puts one persons order in front of another and cant be taken back, while issue_refund is the scariest name on the list and is the recoverable one. it deliberately did not design the gate, checkbox 4 owns that, and it did not touch permissions, checkbox 3 owns that. the next checkbox is injection grows hands, when poisoned text can act. per the overlap note below it opens on 052, where injection makes the model say something, and changes only the consequence, now the same poisoned text reaches a tool call my code runs. 122 gives it the handle it needs, the write rows in that table are exactly what an injected instruction would aim at.

for the record, what 122 established: a four row three column comparison table, the call, what a wrong one leaves behind, can i take it back. rows are search_docs leaving nothing but spent tokens, get_order_by_id with a wrong id leaving one customers order on another ones screen with no, issue_refund leaving money out of the account with yes a second call reverses it, and email_customer leaving a mail somebody already read with no. no arithmetic in it at all, the numbers in this arc are going to be step counts and budgets later. it reuses the made-up tool names the repo already has and adds one, email_customer. its real world detail is a GET handing back the wrong users record still being an incident in my .NET work. it stops, it lands on the sort rule and bookends its own opening line rather than previewing the gate.
...
previously, what 120 established: a mermaid sequence diagram, three participants, model and my code and the screen, six messages running screenshot plus goal in, click 412, 290 out, the real click, a fresh screenshot back, that screenshot in again and then type "invoice", with a note reading one action per round. numbers are 412, 290 and a 40 pixel shift, both arbitrary positions rather than costs. it named 119, 109, 054 and 082. its real world detail is grabbing a button by its test id in my react work. it ended by handing blast radius to arc 11, so it points forward.

so arc 10 is closed and arc 11 is one brick in. the repo now has an agent that can be described to, called, recovered when a call fails, stopped when it thinks its finished and isnt, and kept inside its window by four different moves plus delegation, and as of 122 it has a way to rank its own tools by what a wrong call costs. what it still has nowhere is anything that acts on that ranking. no gate, no scoping, no trace, no budget. those are the next four checkboxes and they should be written as if the table in 122 is already on the readers desk.

overlap to keep an eye on for arc 11: 050 already appends a broken reply plus an error string and retries, for the models own malformed json, and 113 is the same move aimed at a tool that failed. if a note goes near retries again, name the difference explicitly. second one, arc 11 has "grade the end state, did the thing actually happen", and 114 deliberately stayed on the runtime return path, the check my code runs before the loop hands anything back. arc 11s note is measurement after the fact, scoring a finished run the way arc 7 scores an answer, so open it on that difference. third one, 119 said the sub-agent comes back with a sentence you cant audit, and arc 11s "a trace, not a log line" owns the fix, 119 only named the problem. fourth one, 097 already itemized what a log line for one model call needs, so the trace note is about a run of twenty calls rather than about logging fields. fifth one, 052 owns prompt injection where the damage is what the model says, and arc 11s "injection grows hands" is the same attack where the damage is what it does, so that note opens on 052 and changes only the consequence. sixth one, 122 owns blast radius and owns the read vs write correction, so no later note in this arc redefines either, they use the term flat. 122 also deliberately left the gate itself alone, so "the gate that doesnt drown you" still has all of its own material, and it left scoping alone so "cap the tool, not the model" has its.
...
process note: 108 at 314 under the 350 capstone ceiling, 109 at 214, 110 at 232, 111 at 193, 112 at 222, 113 at 189, 114 at 198, 115 at 190, 116 at 187, 117 at 197, 118 at 207, 119 at 203, 120 at 205, 121 at 326. 121 drafted at 339 and came down by trimming five phrases, no brick lost its mention. both capstones landed in the 310 to 330 band and read fine, aim there rather than at the ceiling. 122 at 219.

last visuals: comparison table, four calls by three columns, what a wrong one leaves behind and whether it can be taken back (122), mermaid flowchart assembly diagram, the toolbox and the loop and the two exits, nine nodes (121), mermaid sequence diagram, model and my code and the screen, a screenshot in and a coordinate out over six messages (120). one table and two mermaids in the last three, so the next note may be mermaid again but a second table in a row would be dull. injection grows hands is a before and after of the same text, so a plain text annotated artifact, the poisoned document with the line that acts marked up, is the natural pick.
last exits: stops (122), stops (121), forward (120). two of the last three just stop, so the next note is free either way.
