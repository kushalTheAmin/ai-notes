...
## THREAD
baton: the reasoning checkbox got split first. it was carrying two things, the mechanism and the money, so it became "the model does its working out in the open" and "thinking tokens on the bill", and 042 wrote the first one. that is the only reason arc 4 is now 13 items instead of 12, older note headers still say "of 12" and that is fine, arc 1 already drifted the same way.

042 opened off the one line in 041 that said the loop writes each token from what it already wrote. 041 filed that under bad news, the compounding drift. 042 flipped it into the good news, that the text already written is the only working memory the model has between one token and the next. the visual was a mermaid flowchart, one prompt "37 x 24, minus 118" forking into two paths, the plain model whose very next token is the first digit of the answer so it spells 8 then 7 then 0 with no working out behind it, and the reasoning model spending tokens on 37 x 20 = 740, 37 x 4 = 148, 740 + 148 = 888, 888 - 118 = 770. the click is that there is no scratch variable in there, so a half-finished number gets written down or it is gone. the definition landed last and small, a reasoning model is one trained to take the right hand path on its own without being asked. the caveat that keeps it honest is that the plain path might still get it right, these models have read a lot of arithmetic, it is just committing to a digit it has not computed. cost was deliberately left out, that is 043s whole job.

the next checkbox is "thinking tokens on the bill: paying for output you dont get to read". the handoff is clean and the ground is already laid. 004 built output token billing and 026 built the full parameter read per token, so a pile of hidden thinking is real money and real latency on both counts. the honest beat to aim for is that those tokens land in output_tokens on the usage block even though most apis hand back a summary or nothing at all, so you are paying per token for text you cannot read. the second beat worth having is the 040 collision, a max_tokens ceiling set for the answer can get eaten by the thinking and leave you with an empty or truncated reply. keep it to those two, and keep "which tasks deserve a reasoning model" out, arc 9 owns model selection.

process note: 193 prose words, first note inside the 140 to 190 band since the run started tracking it, and it got there by splitting the checkbox rather than by trimming a finished draft. that is the move to repeat.

last visuals: mermaid diagram (042), worked example (041), comparison table (040). a diagram just ran, so 043 should not repeat one. the annotated artifact has not appeared in a while and a usage block is exactly that shape.
last exits: forward (042), forward (041), stops (040). two of the last three point forward, so 043 has to just stop.
