...
baton: 119 established that a sub-agent is a tool whose body is another loop from 082, running in its own empty message array, so the parent asks a question and its array grows by the returned sentence rather than by the rounds of work behind it. it drew the line against 116 out loud, truncating decides how much of a raw result gets into this window and a sub-agent means the raw result never reaches this window at all. it named the honest price twice, the sub-agents tokens still get paid for once inside its own run and what you skip is 115s replay bill, and the sub-agent starts blind so a vague brief comes back as a confident wrong sentence you cant audit. 120 picks up from exactly there: the tool has been a function and then a whole agent, and computer use makes it a screen, so open on what the tool is this time rather than reopening context size, which arc 10 has now spent four notes on and is done with. the next checkbox is computer use, when the tool is the screen.

for the record, what 119 established: a plain text worked example, two labelled scenarios, inline in the parents own window with three calls at 3,200, 2,900 and 4,100 tokens accumulating to 10,200 of raw hits, against the same job handed to research_agent, its own fresh array holding the same 3 calls and the same 10,200 tokens and going away when it returns, with one ~15 token sentence coming back and the parent array growing by one line. rounds 4 to 6. it named 118, 116, 115 and 082. its real world detail is handing a ticket to someone without replaying every standup. it just stopped, no forward pointer.
...
previously, what 117 established: a plain text annotated artifact, rounds 1 to 9 at 29,700 tokens on top, an arrow labelled with the fold prompt, and underneath the actual 240 token summary message it produced, four lines naming three invoice numbers, one already refunded and a customer id. numbers are 29,700 folded, 240 out, 1,700 for everything else in the array, 31,400 before and 1,940 after against a 32,000 window. it named 053 and 047 out loud and drew the line, both of those slice, folding is the agent taking notes on its own earlier work and it costs a real call. its payoff line is that if the already refunded line drops from the summary, round 12 pays that invoice twice. it just stopped, on the model reading notes about itself.

so as of 119 the arc has a described toolbox, a place to fetch it from, a loop that recovers from a failed call, a done check that does not ask the model, an itemized bill for one round, a rule for how much of a tool result gets in, a way for the run to survive outgrowing the window, somewhere durable to put what must not be summarised away, and a way to run a whole job in a window this one never sees. still no check anywhere on whether a call should happen, arc 11 owns that.

overlap to keep an eye on for 120: the four context notes, 115 through 118 plus 119, have between them said everything this arc will say about what the array costs and how to keep it small. computer use is not another one of those, it is about what a tool can be when there is no api behind it, so keep 120 on the screenshot in and the click out, and on the fact that the model is reading pixels rather than a json result. 119 has now spent 3,200, 2,900, 4,100, 10,200, ~15 tokens, rounds 4 to 6 and research_agent, and 118 spent 800, 8,000, six steps, plan.md, round 3 and round 14, dont reuse either set for a different point. 109 is the nearest neighbour for a tool call that acts on the world, and a click is exactly that, so if 120 goes near blast radius it should hand that off to arc 11 rather than start it.
...
process note: 108 at 314 under the 350 capstone ceiling, 109 at 214, 110 at 232, 111 at 193, 112 at 222, 113 at 189, 114 at 198, 115 at 190, 116 at 187, 117 at 197, 118 at 207, 119 at 203. 119 drafted at 218 and came down by trimming the walkthrough sentence, not the two price caveats, same way round as 118. keep 120 under 210 and treat the 220s as a signal the checkbox wanted splitting.

last visuals: plain text worked example, two labelled scenarios, three calls totting up to 10,200 tokens in the parents window against the same job returning one sentence from a sub-agent (119), mermaid flowchart, one plan forking into the array branch and the file branch, seven nodes (118), plain text annotated artifact, an arrow from a token count into the actual summary message it produced, with before and after totals under it (117). two plain text blocks in the last three, so 120 is not one, and a screenshot in and a click out probably wants a sequence diagram or a table anyway.
last exits: stops (119), forward (118), stops (117). only one of the last three points forward, so 120 may go either way.
