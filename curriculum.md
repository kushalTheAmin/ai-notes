...
baton: 103 closed the three-note data-leaves-the-building run. the visual was a mermaid flowchart, eight nodes, one request forking into a copy in my logs and a copy on their side, my branch carrying fields and expiry, their branch carrying two diamonds, kept how long and trained on, both landing on one node that says account setting and not code. the walk was that every call makes two copies and i keep forgetting the one on my own disk, where 097s log line was quietly holding prompt text with no expiry on it. the click is that retention and training are two separate settings and i had been reading them as one question, a provider can hold a request for a few weeks for abuse checks and train on none of it. the honest half is that the consumer chat app and the api from the same company often answer differently, so one look at a homepage isnt an answer. it closed on this being the first thing in arc 9 i cant fix in my own repo. exit just stopped.

next is "designing for wrong: shipping a feature that fails 5% of the time, on purpose". 103 ended on a limit i cannot code around, so 104 picks up the same acceptance move and turns it inward, the model itself is wrong some of the time and no note in this repo ever promised otherwise. the shape wants a real failure rate turned into what the ui does about it, since 038 already established the made-up answer isnt patchable and 094 already built what the user sees when the call is gone. this is the different case, the call succeeded and the answer is wrong, and nothing threw. do not re-teach hallucination (038) or the retry ladder (089 to 094), and do not turn it into an eval note, arc 7 owns measurement.

caches stay laid and closed, arc 8 owns all of them and 098 owns the assembly, dont let arc 9 reopen a mechanism. bm25 scoring is still an unlaid brick on purpose. the flag 097 planted about logging prompt text is now spent, 103 answered it. 099 deliberately did not name any specific model or provider, and 101, 102 and 103 all held that line, 103 said "a provider" and "the same company" on purpose. model selection later in this arc owns all comparisons.

process note: 097 at 216, 098 at 324 as a capstone, 099 at 217, 100 at 216, 101 at 182, 102 at 206, 103 at 193. the 200s were getting habitual so 101 and 103 came in under. 104 may ride higher, but not three in a row near 250.

last visuals: mermaid flowchart, eight nodes, one fork into two branches with two diamonds on one side (103), worked example, one sentence traced through five labeled steps with a two-row map (102), annotated jsonc request array, four content blocks with one comment each (101). no type repeats yet. 104 must not be mermaid, and a table or a small worked example would both be fresh.
last exits: stops (103), stops (102), forward (101). two of the last three just stop, so 104 should point forward.
