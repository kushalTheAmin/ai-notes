...
## THREAD
baton: 096 took 095s closing line, a snapshot gets retired on a date somebody else picked, and made that date the note. the establishing move is that retirement is a calendar item, not an incident: the provider publishes the date, the announcement arrives as a changelog post or an email to whoever owns the account so it never lands in your logs, and then on one day every call to that id errors at once with a bad model id, which 089 cannot wait out. the click is that the migration window is the whole point and the config edit is the smallest part of it, so the real work is pointing the golden set (072) at the candidate snapshot and reading the diff the way 077 taught. the caveat that some providers route a retired id to its successor for a while is present, tied back to 095 as the same silent move. .NET end of support was the analogy that carried it. the note ended on a question, not a pointer forward.

next is "observability: logging llm calls like http calls". it picks up the shape of both 095 and 096, two failures that never threw anything you could see, and asks what you would have to record to see them at all. the material is what a log line for a model call has to carry that an http log line doesnt, the model id and the token counts and the latency and the prompt version, and the fact that without those fields none of arc 8 is measurable after the fact. it must not re-derive the retry loop (089), the timeout (092), the budgets (087) or the golden set (072). a single annotated log line with its fields called out, or a small table of field against the question it answers, is the natural shape. last two visuals were a mermaid flowchart (096) and an annotated artifact (095), so 097 should take a worked example, a table or pseudocode, not mermaid.

caches stay laid and closed. 085 owns exact prefix caching, 086 owns fuzzy question matching, 087 owns the two budget numbers, 088 owns the meters, 089 owns backoff, 090 owns jitter, 091 owns retryable vs never, 092 owns the timeout number, 093 owns the second provider, 094 owns what the user gets when it is all gone, 095 owns alias vs dated snapshot, 096 owns the retirement date and the migration window. bm25 scoring is still an unlaid brick on purpose, dont let arc 8 or 9 drag it in.

process note: 085 at 179, 086 at 234, 087 at 207, 088 at 223, 089 at 212, 090 at 202, 091 at 196, 092 at 172, 093 at 192, 094 at 201, 095 at 229, 096 at 206. the 176 to 215 band is where this arc reads best.

last visuals: mermaid flowchart, one deprecation window from the pin to the retirement day, branching into the migration loop and into the do nothing path (096), annotated artifact, one model field written both ways and then resolved on two dates, with the alias moving and the snapshot not (095), comparison table, five things you can serve when everything is gone, against what the user sees and what it costs to build (094).
last exits: stops (096), forward (095), forward (094). 097 may point forward again.
