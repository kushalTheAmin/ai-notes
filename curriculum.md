...
## THREAD
baton: 080 took 079s closing line, that the six failures only existed because i wrote the questions, and answered where failures come from once real users are typing. what it settled: the two doors a production failure can arrive through, and that only one of them is free. thumbs down costs nothing to collect and it came back 11 times on 4,200 answers, of which 6 were really bad, so waiting at that door hands you almost nothing. reading 50 answers picked at random, none of them complained about, found 7, which puts the real number somewhere near 600 in the same week. the landing is who doesnt click, the person who reads a confident wrong paragraph and believes it, and that person never files anything. the close is the loop back, a bad answer found in the wild becomes a new row in golden.jsonl (072) and 078 then re-checks that row on every prompt commit, with 072s ordering rule honoured, add the rows first and re-run the old prompt after. it deliberately built nothing about online A/B, shadow traffic or drift.

next is the arc 7 CAPSTONE, an eval harness for the arc 6 system. its job is assembly, not new material, and it has to link every brick by number: 071 the cost of a person reading, 072 the golden file, 073 the two mechanical checks, 074 the judge, 075 where the judge tilts, 076 the wobble, 077 the diff instead of the total, 078 the ci gate on rows, 079 the piles, 080 where new rows come from. the shape to aim for: one picture of the harness as parts, offline file plus scorers plus gate plus the live loop feeding rows back in, each part labelled with the note that built it, and a capstone budget up to 350. run the CAPSTONE GAP CHECK first, the one thing worth checking for is whether anything about running the harness itself was ever laid down, and if a brick really is missing, add exactly one checkbox before the capstone and write that instead.

pairwise A/B judging stays banned in this arc, the capstone must not smuggle it in as an assembly part.
...
process note: 070 at 324 on the capstone ceiling, 071 at 190, 072 at 221, 073 at 191, 074 at 202, 075 at 218, 076 at 200, 077 at 208, 078 at 215, 079 at 208, 080 at 217. 080 drafted at 229 and two small cuts brought it to 217, both of them words the diagram was already saying. the arc has now sat between 190 and 221 for ten notes running, which is a flatness worth breaking, so the capstone should use its ceiling properly rather than land at 220 like everything else. 081 is the capstone, 280 to 350.

last visuals: mermaid flowchart, one weeks traffic splitting into thumbs and a random sample and both ending in a golden row the ci gate then watches (080), worked list, six failing rows grouped under three named pile headings with a count line (079), pseudocode, a ci job that computes the regressed rows and exits on them (078). the three-in-a-fence run is broken. capstone visuals are assembly diagrams by rule, so 081 is a mermaid or a labelled parts picture either way, just not a second flowchart shaped like 080s.
last exits: stops (080), forward (079), stops (078). no run to worry about, so 081 may do either, though a capstone closing an arc probably wants to point at arc 8.
