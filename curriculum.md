...
## THREAD
baton: 093 wrote the line after the last timeout. the establishing move is that give up is a branch, not an end: the retry loop running out of attempts hands off to one more call, a different model at a different provider, same question different door. the click is that this is not a spare url in an env file, its a second integration, model B has its own price, its own speed and its own idea of what json looks like, so 050s validation now guards two shapes, and the prompt and every arc 7 eval row were written against model A. so the fallback path is the only path never tested and it runs on the worst day. the ask laid down is to point the golden set (072) at the backup. the caveat is laid: fallback is for the provider being down, a 400 from your own bad request (091) is a 400 at model B too. 093 deliberately said nothing about what the user sees when the fallback also dies.

next is "failing gracefully: what the user gets when every attempt is gone". it starts exactly where 093 stopped, at the nothing left to serve box in 093s diagram. the material is the product decision, not the plumbing: a cached answer from 086, a degraded answer with the retrieved chunks and no model, a plain honest error, or a queue and come back later, and the fact that a raw 500 or a spinner that never ends is itself a choice you made by not making it. it must not re-derive the retry loop (089), the timeout number (092) or the second provider (093). a comparison table of what the user actually sees per option, with what it costs you to build, is the natural shape and the last table was 091, three notes back, so it is free. avoid another mermaid diagram right after 093.

caches stay laid and closed. 085 owns exact prefix caching, 086 owns fuzzy question matching, 087 owns the two budget numbers, 088 owns the meters, 089 owns backoff, 090 owns jitter, 091 owns retryable vs never, 092 owns the timeout number, 093 owns the second provider. bm25 scoring is still an unlaid brick on purpose, dont let arc 8 or 9 drag it in.

process note: 085 at 179, 086 at 234, 087 at 207, 088 at 223, 089 at 212, 090 at 202, 091 at 196, 092 at 172, 093 at 192. the 176 to 215 band is where this arc reads best and 093 sat mid band with the diagram carrying the branch and the prose carrying the cost.

last visuals: mermaid flowchart, one call with the retry branch feeding a second provider and a dotted node naming what is not the same about it (093), worked example, four timeout choices with the calls and the sleeps added up to what the user actually waits (092), comparison table, seven status codes with retry or never and the reason in the third column (091). so 094 should not be a mermaid diagram, and a table is fine again.
last exits: stops (093), stops (092), forward (091). two stops running, so 094 may end either way but pointing forward would read fresh.
