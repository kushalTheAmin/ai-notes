...
baton: 117 established that a long run outgrows the window mid-flight and that the fix is a fold, one extra model call that turns the oldest rounds into a paragraph which then sits in the array where they were. the fold is lossy and one way, so the fold prompt has to name what must survive, ids, filenames, decisions. 118 picks up from exactly there: the paragraph is still tokens in the array, still costing on every round, and still able to drop the one line that mattered. the next checkbox is the plan is just tokens, files are the agents real memory, so open it on the fact that a summary you cant reread is not storage, and a file the agent writes to and reads back is. dont reopen the window running out, 117 has spent that.

for the record, what 117 established: a plain text annotated artifact, rounds 1 to 9 at 29,700 tokens on top, an arrow labelled with the fold prompt, and underneath the actual 240 token summary message it produced, four lines naming three invoice numbers, one already refunded and a customer id. numbers are 29,700 folded, 240 out, 1,700 for everything else in the array, 31,400 before and 1,940 after against a 32,000 window. it named 053 and 047 out loud and drew the line, both of those slice, folding is the agent taking notes on its own earlier work and it costs a real call. its payoff line is that if the already refunded line drops from the summary, round 12 pays that invoice twice. it just stopped, on the model reading notes about itself.

previously, what 116 established: a 12 line python block, one index.search call and then two labelled branches, truncate and paginate, each keeping hits 0 to 4 and each carrying a one line comment on what happens to the other 43. it named 053 out loud and had to. it just stopped, no forward pointer.

so as of 117 the arc has a described toolbox, a place to fetch it from, a loop that recovers from a failed call, a done check that does not ask the model, an itemized bill for one round, a rule for how much of a tool result gets in, and a way for the run to survive outgrowing the window. still no check anywhere on whether a call should happen, arc 11 owns that.

overlap to keep an eye on for 118: 117 just made the summary the agents memory, so 118 has to be honest that its arguing against something the previous note built, not adding a second unrelated trick. the difference is rereadability and cost, a folded paragraph is one shot and rides in the array forever, a file can be written once and read back by name only when needed. 053 stays the nearest neighbour for anything about what goes in the array, name it if 118 goes near budgeting. also 117 has now spent 29,700, 240, 31,400, 1,940 and the INV-18/19 numbers, dont reuse those for a different point.

overlap to keep an eye on: 050 already appends a broken reply plus an error string and retries, for the models own malformed json. 113 is the same move aimed at a tool that failed, inside the agent loop. if a later note in this arc goes near retries again, name the difference explicitly. second one, arc 11 has "grade the end state, did the thing actually happen". 114 deliberately stayed on the runtime return path, the check my code runs before the loop hands anything back. arc 11s note is measurement after the fact, scoring a finished run the way arc 7 scores an answer. when that note gets written, open it on that difference.
...
process note: 108 at 314 under the 350 capstone ceiling, 109 at 214, 110 at 232, 111 at 193, 112 at 222, 113 at 189, 114 at 198, 115 at 190, 116 at 187, 117 at 197. five short ones in a row, all landed, all by picking a concept with one moving part rather than by trimming. keep 118 under 210 and treat the 220s as a signal the checkbox wanted splitting.

last visuals: plain text annotated artifact, an arrow from a token count into the actual summary message it produced, with before and after totals under it (117), 12 line python block, one search call and two labelled branches with a comment each on what happens to the rest (116), markdown table, three columns, one rounds payload itemized by line and by who wrote it (115). nothing is three running, so 118 is open, but a mermaid diagram has not appeared since 112 and is the freshest option.
last exits: stops (117), stops (116), forward (115). two of the last three just stop, so 118 may go either way.

process note on visuals: mermaid stacks subgraphs in whatever order it likes and it flipped the two on 065, and on 070 it put the once-per-doc group beside the per-question one rather than above it. so 070s prose says "one group" and "the other group" instead of top and bottom, and 098 names its groups seconds, minutes and months for the same reason. do not write positional references into prose about a mermaid block, github may lay it out differently than a local render. 106 hit this while drafting, "that first fork" became "the fact question" so the prose names the node by its content instead of its place. 108 hit it twice, "the first fork" became "the rule question" and "every other question below" became "every other question here", both for the same reason. tables are safe for this, 099 says "top row" and "row two" and github renders rows in file order, and 104 leans on top two rows and bottom two rows the same way. plain text blocks are safe too, 102 labels each step in file order and github shows them that way, 109 says top block and bottom block for the same reason, and 113 leans its whole payoff on "line 3" of a numbered block. code blocks are safe the same way, 116 says the truncate branch and the paginate branch by their labels anyway. sequence diagrams are safer than flowcharts here, 112 rendered with its participants in declaration order, but its prose still names the actors instead of saying left or right. mmdc runs in this container only with a puppeteer config passing --no-sandbox, without it the render dies on a root sandbox error that has nothing to do with the diagram.
