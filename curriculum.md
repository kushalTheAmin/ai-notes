...
## THREAD
baton: 094 filled in the box 093 left blank, nothing left to serve. the establishing move is that when the retries and the backup provider are both gone your code still returns something, and which something is a product decision, not error handling. the click is that skipping the decision still picks a row, the worst one, since a spinner that never resolves is what the code does when nobody chose. the two rows that cost almost nothing are already built and paid for: the semantic cache from 086 likely holds a near match, and in doc-QA the retrieval from 059 finished before the model call died so the chunks are still in a variable, and three source paragraphs with no summary answers plenty. the rule laid down is label the degraded answer, a stale one served silently as fresh is worse than the error. 094 deliberately said nothing about the model that never goes down but gets deprecated.

next is "the model changes under you: pinning versions and surviving deprecations". it starts from 094s last line, the failure that is not an outage. the material is the model id you pinned or didnt, a dated snapshot vs a floating alias, the fact that the same prompt on a new snapshot is a silent behaviour change your arc 7 golden set is the only thing that catches, and a deprecation date being a calendar item rather than an incident. it must not re-derive the fallback provider (093), the retry loop (089) or the degraded-answer menu (094). a comparison of the same call pinned vs floating, or a dated timeline of a deprecation window, is the natural shape. the last table was 094 and the last mermaid was 093, so 095 wants a worked example, an annotated artifact or pseudocode.

caches stay laid and closed. 085 owns exact prefix caching, 086 owns fuzzy question matching, 087 owns the two budget numbers, 088 owns the meters, 089 owns backoff, 090 owns jitter, 091 owns retryable vs never, 092 owns the timeout number, 093 owns the second provider, 094 owns what the user gets when it is all gone. bm25 scoring is still an unlaid brick on purpose, dont let arc 8 or 9 drag it in.

process note: 085 at 179, 086 at 234, 087 at 207, 088 at 223, 089 at 212, 090 at 202, 091 at 196, 092 at 172, 093 at 192, 094 at 201. the 176 to 215 band is where this arc reads best and 094 sat mid band with the table carrying the five options and the prose carrying the two free rows.

last visuals: comparison table, five things you can serve when everything is gone, against what the user sees and what it costs to build (094), mermaid flowchart, one call with the retry branch feeding a second provider and a dotted node naming what is not the same about it (093), worked example, four timeout choices with the calls and the sleeps added up to what the user actually waits (092). so 095 should not be a table and a mermaid diagram is still cooling off.
last exits: forward (094), stops (093), stops (092). 094 broke the run of stops, so 095 may end either way.
