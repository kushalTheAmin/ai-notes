...
baton: 099 opened arc 9 by asking whether a job needs a model at all. the visual was one support inbox and five jobs in a three column table, the job, whether a rule decides it, and what i build. the top rows have the answer already sitting in the text so a rule finds it every time, and handing those to a model buys a slow expensive regex that bills per call (087), varies run to run (041) and can invent an id nobody typed (038). the middle row is the honest one, a keyword router that works until a product gets renamed or somebody writes in gujarati, and the duplicate row goes to embeddings and a threshold (019) rather than to either extreme. the landing was that "can a model do this" was never the question, a model can do all five rows, it just does the top one worse than a regex. the test the note leaves behind is whether the list of cases stays closed, and that hard has nothing to do with it. the note pointed forward, to the rows where a model really is the answer and what has to go around it.

next is "content moderation: the filter in front of and behind the model". it picks up 099s last line directly, since 099 sorted the jobs a model should keep and this is the first thing you put around one of those jobs before real users touch it. two checks, one on what comes in and one on what goes out, and the note must stay a decision note, not a mechanism note. it can lean on 052 for hostile user input and on 018 for the fact that a classifier can be the filter, but it must not re-teach either. keep the examples concrete and keep the failure honest, a filter has both kinds of miss and you pick which one you would rather ship.
...
process note: 093 at 192, 094 at 201, 095 at 229, 096 at 206, 097 at 216, 098 at 324 as a capstone, 099 at 217. the 176 to 215 band is where the regular notes read best, 099 came in just over it after a trim from 244.

last visuals: comparison table, three columns and five rows, one job per row from a single support inbox (099), mermaid assembly diagram, three labelled subgraphs for the three clocks (098), annotated artifact, one json log line for a model call (097). no mermaid in the last two, so 100 may use one.
last exits: forward (099), stops (098), forward (097). 100 must stop, two of the last three already point forward.
