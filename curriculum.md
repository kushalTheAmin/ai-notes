...
baton: 098 closed arc 8 by assembling every brick from 082 to 097 into one deployed doc-QA feature, drawn as three clocks. seconds is the good-day request path, cache check (085, 086), the capped loop (082, 083), streamed out at 870ms felt against 2,870 total (084, 087). minutes is the same code failing, the retryable question (091, 092), wait and double and jitter (088, 089, 090), out of tries goes to a second provider (093), a bad key skips that branch entirely and lands on what you serve when nothing answered (094). months is the path that breaks with nobody touching the code (095, 096) with the golden set pointed at the replacement (081). the log line (097) sits under all three and its model field is what says which clock you are in. the landing was that every box in the picture is code i wrote and the model call is one line in the middle of it, so the arc filed in my head as the ai part is really caching, retries, timeouts, version pinning and logging. the note stopped, it did not point forward.

next is arc 9 note 001, "should this even be an llm: when boring code wins". it is an arc opener, so it builds on 098, and the connection is already sitting there: 098 counted sixteen notes of scaffolding around one model call, which is the honest price of shipping one. that price is the reason the first question in arc 9 is whether you should pay it at all. the note has to stay concrete about what boring code wins at, a regex, a lookup table, a classifier trained on labels you already have, and it must not re-teach anything from arc 8. arc 9 is decisions, not mechanisms.

caches stay laid and closed. 085 owns exact prefix caching, 086 owns fuzzy question matching, 087 owns the two budget numbers, 088 owns the meters, 089 owns backoff, 090 owns jitter, 091 owns retryable vs never, 092 owns the timeout number, 093 owns the second provider, 094 owns what the user gets when it is all gone, 095 owns alias vs dated snapshot, 096 owns the retirement date and the migration window, 097 owns the log line fields for a model call, 098 owns the whole-arc assembly. bm25 scoring is still an unlaid brick on purpose, dont let arc 9 drag it in. pii and redaction belong to arc 9 and 097 already flagged them forward, so the arc 9 note that owns them picks up that flag.

process note: 085 at 179, 086 at 234, 087 at 207, 088 at 223, 089 at 212, 090 at 202, 091 at 196, 092 at 172, 093 at 192, 094 at 201, 095 at 229, 096 at 206, 097 at 216, 098 at 324 as a capstone. the 176 to 215 band is where the regular notes read best.

last visuals: mermaid assembly diagram, three labelled subgraphs for the three clocks with every note number 081 to 097 sitting inside a box (098), annotated artifact, one json log line for a model call with the http fields on top and the model-specific fields under a gap (097), mermaid flowchart, one deprecation window from the pin to the retirement day (096). two mermaid in the last three, so 099 should not be mermaid.
last exits: stops (098), forward (097), stops (096). 099 may point forward.
