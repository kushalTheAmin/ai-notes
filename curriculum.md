...
## THREAD
baton: 040 turned the camera around. after 037, 038 and 039 spent three notes on what the loop will not tell you, 040 is the part that works: how the loop ends. it took back the one thing 037 named and nobody had built on, the real done token that wins a row like any other token and breaks the loop from 022, and put the two outside cuts next to it. the visual was a three row comparison table on one prompt, "name 3 gujarati snacks, numbered", each row a different run: nothing set and the done token wins, returning all four snacks because it gave me one more than i asked for, which is what motivates the second row; stop_sequences ["4."], returning three snacks; max_tokens 12, returning "1. dhokla 2. fa". third column is stop_reason: end_turn, stop_sequence, max_tokens. the mechanism beat is that the two cuts work nothing alike. a stop sequence is not something the model knows about, it generated the "4." happily and the api matched the string against the text on the way out, so that check lives outside the model. max tokens never reads the text at all, it counts and cuts, mid word if thats where the ceiling lands, and the note carries a caveat that exactly where a count lands depends on the split from 003. the honest beat that makes it land: a max_tokens stop still looks like an answer, and sometimes the cut falls after a full stop and reads finished, so the only place the difference exists is the flag, stop_reason on anthropics api, finish_reason on openais. the note ends flat on not being able to tell a cut answer from a finished one by reading it. streaming was kept out, arc 8 has it. the next checkbox is "determinism: why the same prompt varies and what that breaks" and it has a clean handoff: 034 established that sampling rolls a number, and 040 just showed that the same prompt can hand back three different strings from knobs alone. determinism should pick up the roll from 034 and say why two identical calls diverge even with the knobs pinned, and what that breaks for a normal engineer, caching, tests, and reproducing a bug report. temperature 0 from 033 is the obvious lever to reach for and the honest beat is that it gets close but is not a guarantee. one process note for the next run: prose word counts have sat at 215 to 233 for five notes running (036 through 040). aim 140 to 190 next time, the ceiling is not the target.

last visuals: comparison table (040), pseudocode (039), mermaid flowchart (038). a table just ran, so 041 must not use one, and it cannot repeat pseudocode either without making it three of four. a worked example with real numbers has not appeared since 037 and fits determinism well, two identical calls laid side by side.
last exits: stops (040), stops (039), forward (038). three of the last four just stop, so 041 should point forward.
