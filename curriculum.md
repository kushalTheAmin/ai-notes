...
baton: 107 laid the model-selection brick. the visual was a three column table, three candidates down the rows, where each one sits on a public ranking against how many of my 50 it passed, and the order inverts, 1st scores 39 while 4th scores 44 and 12th scores 43. the walk was that a public rank is one average over a huge mixed bag and my narrow task barely registers in it, so the top row losing is not a scandal, and the reason was concrete, the big one kept wrapping the json in a friendly sentence and the grader counted those wrong. then the turn, 44 and 43 out of 50 is not a real gap (076) so quality has stopped deciding and the two numbers from 087 decide instead. it landed on the method already being built, its the golden set (072), change the id, rerun, read the score. exit stops.

next is the arc 9 capstone, "the checklist i would run before shipping any llm feature". it assembles 099 through 107 and its job is assembly only, every brick linked by note number, no new mechanism. the shape it wants is the order you actually ask the questions in, should this be a model call at all (099), then the gates and the privacy decisions (100 to 103), then what wrong costs and where it lands (104), then the three picks, rent or own (105), which lever (106), which model (107). capstone budget is up to 350 prose words and the visual is an assembly diagram with a note number on every label. run the capstone gap check first: if the assembly needs a brick nobody laid, add one checkbox and write that brick instead.

still no named vendor and no named benchmark, the arc has held that line since 099 and the capstone must hold it too, keep it to shapes. caches stay laid and closed, arc 8 owns all of them and 098 owns the assembly, dont let arc 9 reopen a mechanism. bm25 scoring is still an unlaid brick on purpose. arc 7 owns measurement, 107 pointed at 072 and 076 in one clause each and that is the right amount. note that this capstone is the last item in the curriculum as it stands, so it has nothing to point forward to, let it close the arc rather than tease.

process note: 100 at 216, 101 at 182, 102 at 206, 103 at 193, 104 at 233, 105 at 220, 106 at 198, 107 at 205. the cluster is loose and steady. the capstone may run long, up to 350, but it does not have to.

last visuals: table, three rows and three columns, public rank against my own score with the order inverting (107), mermaid flowchart, eight nodes, three diamonds chained down with a cost label on every lever (106), plain text bill, the same day costed twice with three traffic rows each and a crossover line (105). a mermaid assembly diagram is fine for the capstone, two apart from 106, and it is the natural shape for assembly.
last exits: stops (107), forward (106), stops (105). the capstone may go either way on the rule, but it is the last note in the curriculum, so it should stop.
