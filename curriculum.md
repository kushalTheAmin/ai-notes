...
baton: 118 established that anything which must still be true late in a run belongs outside the message array, because a plan left in the array is replayed on every round and can be rewritten by 117s fold, while a plan written to a file costs its tokens once on the write, once on the read, and comes back word for word. it named the honest price out loud, reading the file back costs the same tokens again on that round, and it closed on the toolbox from 110 needing a read and a write in it before any of this exists. 119 picks up from exactly there: files are one way to keep the array small, a second agent with its own fresh window is the other, so open 119 on that contrast rather than reopening what a full window costs. the next checkbox is sub-agents, a fresh window with a narrower job, and 118 has already pointed at it by name.

for the record, what 118 established: a mermaid flowchart, seven nodes, one plan of 800 tokens forking into left in the array and written to plan.md, the array branch running to resent every round and then the fold can rewrite it, the file branch running to array keeps the filename and then read back word for word. numbers are 800 tokens for the plan, written on round 3, ten more rounds of replay at 8,000 tokens, six steps, read back on round 14. it named 115 for the replay bill and 117 for the fold. its real world detail is a grocery list that drifts in my head and doesnt on paper. it ended pointing forward at 119 in one short clause.
...
so as of 118 the arc has a described toolbox, a place to fetch it from, a loop that recovers from a failed call, a done check that does not ask the model, an itemized bill for one round, a rule for how much of a tool result gets in, a way for the run to survive outgrowing the window, and somewhere durable to put what must not be summarised away. still no check anywhere on whether a call should happen, arc 11 owns that.

overlap to keep an eye on for 119: 118 just argued for moving state out of the array, and a sub-agent is the other way to keep the array small, so 119 has to be clear that these two are not the same trick. a file moves one artefact out and leaves the run in one window, a sub-agent moves a whole job out into its own window and returns only a result. if 119 needs a handoff between the two, the file is the obvious channel, but dont let that swallow the note. 118 has now spent 800, 8,000, six steps, plan.md, round 3 and round 14, dont reuse those numbers for a different point. 053 stays the nearest neighbour for anything about what goes in the array.
...
process note: 108 at 314 under the 350 capstone ceiling, 109 at 214, 110 at 232, 111 at 193, 112 at 222, 113 at 189, 114 at 198, 115 at 190, 116 at 187, 117 at 197, 118 at 207. 118 drafted at 224 and came down to 207 by cutting filler rather than the caveat, which is the right way round. keep 119 under 210 and treat the 220s as a signal the checkbox wanted splitting.

last visuals: mermaid flowchart, one plan forking into the array branch and the file branch, seven nodes (118), plain text annotated artifact, an arrow from a token count into the actual summary message it produced, with before and after totals under it (117), 12 line python block, one search call and two labelled branches with a comment each on what happens to the rest (116). nothing is three running, so 119 is open, but not another mermaid flowchart back to back.
last exits: forward (118), stops (117), stops (116). only one of the last three points forward, so 119 may go either way.
